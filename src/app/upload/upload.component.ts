...
async loadFinishedSessions() {
  this.finishedUploads = await this.http.get<any[]>('/upload/finished').toPromise();
}
...
onFileSelected(event: Event): void {
  ...
  // If we have a resume session and file matches
  if (
    this.file &&
  ...
  async loadIncompleteSessions() {
    try {
      this.incompleteUploads = await this.http.get<any[]>('/upload/unfinished').toPromise();
    } catch (err) {
  ...
  async startUpload(): Promise<void> {
    ...
    const initRes = await this.http.post<any>(
      `/upload/init?fileName=${encodeURIComponent(this.file.name)}&totalSize=${this.file.size}`,
      {}
    ).toPromise();
  ...
  async uploadChunks(uploadId: string, file: File, fromChunk: number, knownUploadedChunks: number[] = []) {
    const totalChunks = Math.ceil(file.size / this.CHUNK_SIZE);

    const uploadedChunks: number[] = knownUploadedChunks.length
      ? knownUploadedChunks
      : await this.http.get<number[]>(`/upload/${uploadId}/uploadedChunks`).toPromise();
    ...
    for (let i = fromChunk; i < totalChunks; i++) {
      ...
      await this.http.patch(`/upload/${uploadId}/chunk`, formData, {
        responseType: 'text'
      }).toPromise();
...
async resumeOldSession(session: any) {
  ...
  const uploadedChunks: number[] = await this.http.get<number[]>(
    `/upload/${this.uploadId}/uploadedChunks`
  ).toPromise();

  this.progress = Math.floor(
    (uploadedChunks.length / Math.ceil(session.totalSize / this.CHUNK_SIZE)) * 100
  );

  this.log.push(`📂 Loaded session for file: ${session.fileName}`);

  // Check if a matching file is already selected
  if (this.file && this.file.name === session.fileName) {
    this.log.push(`▶️ Resuming upload with selected file: ${this.file.name}`);
    this.uploadChunks(this.uploadId, this.file, this.currentChunkIndex, uploadedChunks);
  } else {
    this.log.push(`⚠️ File not selected or mismatch. Please reselect: ${session.fileName}`);
    this.activeResumeSession = {
      session,
      uploadedChunks
    };
